...
var EpixTestingAppInit func() (TestingApp, map[string]json.RawMessage) = SetupTestingEpixApp
...
func SetupTestingEpixApp() (TestingApp, map[string]json.RawMessage) {
	db := dbm.NewMemDB()
	app := epixapp.NewEpix(log.NewNopLogger(), db, nil, true, map[int64]bool{}, epixapp.DefaultNodeHome, 5, false, simtestutil.EmptyAppOptions{})

	return app, epixapp.NewDefaultGenesisState()
}
...
// of one consensus engine unit (10^6) in the default token of the simapp from first genesis
// account. A Nop logger is set in SimApp.
func SetupWithGenesisValSetEpix(tb testing.TB, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, chainID string, balances ...banktypes.Balance) TestingApp {
	tb.Helper()
	app, genesisState := DefaultTestingAppInit()
	...
	_, err = app.InitChain(
		&abci.RequestInitChain{
			ChainId:         chainID,
			Validators:      []abci.ValidatorUpdate{},
			ConsensusParams: epixapp.DefaultConsensusParams,
			AppStateBytes:   stateBytes,
		},
	)
	require.NoError(tb, err)
