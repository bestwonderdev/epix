...
// ChainIDPrefix defines the default chain ID prefix for epix test chains
var ChainIDPrefixEpix = "epix_4243-"
...
// Time management is handled by the Coordinator in order to ensure synchrony between chains.
// Each update of any chain increments the block header time for all chains by 5 seconds.
func NewTestChainEpix(t *testing.T, coord *Coordinator, chainID string) *TestChain {
	t.Helper()
	...
	app := SetupWithGenesisValSetEpix(t, valSet, []authtypes.GenesisAccount{acc}, chainID, balance)
