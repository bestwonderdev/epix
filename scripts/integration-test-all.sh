...
DATA_DIR=$(mktemp -d -t epix-datadir.XXXXX)
...
init_func() {
    "$PWD"/build/epixd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/epixd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/epixd add-genesis-account \
    "$("$PWD"/build/epixd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aepix,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/epixd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/epixd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/epixd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    echo "starting epix node $i in background ..."
    "$PWD"/build/epixd start --pruning=nothing --rpc.unsafe \
    ...
    epix_PID=$!
    echo "started epix node, pid=$epix_PID"
    # add PID to array
    arr+=("$epix_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test epix node $HOST_RPC ..."
...
stop_func() {
    epix_PID=$i
    echo "shutting down node, pid=$epix_PID ..."
