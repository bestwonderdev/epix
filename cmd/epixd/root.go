...
// NewRootCmd creates a new root command for epixd. It is called once in the
// main function.
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	initClientCtx := client.Context{}.
		...
		WithKeyringOptions(epixkr.Option()).
		WithViper(EnvPrefix)
...
func (a appCreator) appExport(
	...
	if height != -1 {
		epixApp = app.NewEpix(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), false, appOpts)

		if err := epixApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		epixApp = app.NewEpix(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), false, appOpts)
	}

	return epixApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs, modulesToExport)
}
