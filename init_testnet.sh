...
# Set client config
epixd config keyring-backend $KEYRING
epixd config chain-id $CHAINID

# if $KEY exists it should be deleted
epixd keys add $KEY1 --keyring-backend $KEYRING --algo $KEYALGO
epixd keys add $KEY2 --keyring-backend $KEYRING --algo $KEYALGO
epixd keys add $KEY3 --keyring-backend $KEYRING --algo $KEYALGO
...
cat $HOME/.epixd/config/genesis.json | jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="aepix"' > $HOME/.epixd/config/tmp_genesis.json && mv $HOME/.epixd/config/tmp_genesis.json $HOME/.epixd/config/genesis.json
...
cat $HOME/.epixd/config/genesis.json| jq '.app_state.gov.voting_params.voting_period="30s"' > $HOME/.epixd/config/tmp_genesis.json && mv $HOME/.epixd/config/tmp_genesis.json $HOME/.epixd/config/genesis.json
...
epixd add-genesis-account $KEY1 1050000000000000000000000000aepix --keyring-backend $KEYRING
epixd add-genesis-account $KEY2 1000000000000000000000000000aepix --keyring-backend $KEYRING
epixd add-genesis-account $KEY3 1000000000000000000000000000aepix --keyring-backend $KEYRING
...
mkdir $HOME/.epixd/config/gentx
epixd gentx $KEY1 900000000000000000000000aepix --keyring-backend $KEYRING --chain-id $CHAINID --output-document $HOME/.epixd/config/gentx/gentx-1.json
#epixd gentx $KEY2 1000000000000000000000aepix --keyring-backend $KEYRING --chain-id $CHAINID
...
#epixd start --pruning=nothing --trace --log_level trace --minimum-gas-prices=1.000aepix --json-rpc.api eth,txpool,personal,net,debug,web3 --rpc.laddr "tcp://0.0.0.0:26657" --api.enable true --api.enabled-unsafe-cors true
