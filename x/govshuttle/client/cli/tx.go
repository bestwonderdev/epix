...
func NewLendingMarketProposalCmd(ac addresscodec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Submit a proposal for the Epix Lending Market along with an initial deposit.
Upon passing, the
...
func NewTreasuryProposalCmd(ac addresscodec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		Short: "Submit a proposal to the Epix Treasury",
		Long: `Submit a proposal for the Epix Treasury along with an initial deposit.
Upon passing, the
The proposal details must be supplied via a JSON file.`,
