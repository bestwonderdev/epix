...
func (suite *TransferTestSuite) SetupTest() {
	...
	suite.chainB = suite.coordinator.GetChain(ibctesting.GetChainIDEpix(1))
}
...
func (suite *TransferTestSuite) TestHandleMsgTransfer() {
	...
	coins := sdk.NewCoins(
		...
		sdk.NewCoin("aepix", sdkmath.NewInt(10000000000)),
	)
	err := suite.chainB.App.(*app.Epix).BankKeeper.MintCoins(suite.chainB.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = suite.chainB.App.(*app.Epix).BankKeeper.SendCoinsFromModuleToAccount(suite.chainB.GetContext(), inflationtypes.ModuleName, suite.chainB.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)

	// Setup params for coinswap
	coinswapKeeper := suite.chainB.App.(*app.Epix).GetCoinswapKeeper()
	coinswapKeeper.SetStandardDenom(suite.chainB.GetContext(), "aepix")
	...
	middlewareParams := suite.chainB.App.(*app.Epix).GetOnboardingKeeper().GetParams(suite.chainB.GetContext())
	middlewareParams.AutoSwapThreshold = sdkmath.NewInt(4000000)
	suite.chainB.App.(*app.Epix).GetOnboardingKeeper().SetParams(suite.chainB.GetContext(), middlewareParams)

	erc20Keeper := suite.chainB.App.(*app.Epix).GetErc20Keeper()
	...
	balanceVoucherBefore := suite.chainB.App.(*app.Epix).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), voucherDenomTrace.IBCDenom())
	balanceEpixBefore := suite.chainB.App.(*app.Epix).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), "aepix")
	...
	balanceVoucher := suite.chainB.App.(*app.Epix).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), voucherDenomTrace.IBCDenom())
	balanceEpix := suite.chainB.App.(*app.Epix).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), "aepix")
	...
	// check whether the epix is swapped and the amount is greater than the threshold
	if balanceEpixBefore.Amount.LT(middlewareParams.AutoSwapThreshold) {
		suite.Require().Equal(balanceEpix.Amount, balanceEpixBefore.Amount.Add(middlewareParams.AutoSwapThreshold))
	} else {
		suite.Require().Equal(balanceEpix.Amount, balanceEpixBefore.Amount)
	}
	...
	blockedAddr := "epix10d07y265gmmuvt4z0w9aw880jnsr700jg5j4zm" // gov module
	...
	suite.Require().Equal(balanceEpixBefore, balanceEpix)
	suite.Require().Equal(balanceVoucherBefore, balanceVoucher)
