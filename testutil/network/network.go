...
func DefaultConfig() Config {
	...
	chainID := fmt.Sprintf("epix_%d-1", tmrand.Int63n(9999999999999)+1)
	return Config{
		Codec:             encCfg.Codec,
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "epixd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "epixcli")
